...
room_router: APIRouter = APIRouter(prefix='/room'  , tags=['Room CRUD'])
...
@room_router.put('/{room_router}_id}/', response_model=dict)
async def update_room(room_id: int,room: RoomInputSchema,
    ...
    for room_key, room_value in room.dict().items():
        setattr(room_key, room_value, room_db)
...
@room_router.delete('/{room}_id}/', response_model=dict)
async def delete_room(room_id, db: Session = Depends(get_db)):
    room_db = db.query(Room).filter(Room.id==room_id).first()
    if not room_db:
        raise HTTPException(detail='Мындай маалымат жок', status_code=400)
